...
__global__ void isInsidePolygon(float p1_x, float p1_y, float p2_x, float p2_y, float p3_x, float p3_y, float p4_x, float p4_y, float *x, float *y, int *flag, int n){
	...
	__shared__ int temp1, temp2, temp3, temp4;
...
__global__ void isInsideTriangle(float p1_x, float p1_y, float p2_x, float p2_y, float p3_x, float p3_y, float *x, float *y, int *flag, int n){
	...
	__shared__ int temp1, temp2, temp3;
...
int main(){
	...
	*/
	...
	/*
	/ STEP 2
	/ Extraction of 2 points, the points with the maximum and the minumum x and y values.
	...
	*/
	pStart = seconds();
	computeFlagsFromLine<<<blocksPerGrid, blocks>>>(x, y, flag, p_left, p_right, N);
	CHECK(cudaDeviceSynchronize());
	pStop = seconds() - pStart;
	printf("\n\tSTEP 6: %f sec \n\n", pStop);
	...
	*/
	pStart = seconds();
	thrust::stable_partition(thrust::device, x + 1, x + (N - 1), flag + 1, is_one());
	thrust::stable_partition(thrust::device, y + 1, y + (N - 1), flag + 1, is_one());
	pStop = seconds() - pStart;
	printf("\n\tSTEP 7: %f sec \n\n", pStop);
	...
	*/
	pStart = seconds();
	int temp_count = thrust::reduce(thrust::device, flag + 1, flag + (N - 1));
	int segm = N - ((N - temp_count) / 2);
	pStop = seconds() - pStart;
	printf("\n\tSTEP 8: %f sec \n\n", pStop);
	...
	*/
	pStart = seconds();
	convexHull(p_left, p_right, x, y, flag, dist, 1, segm-1);
	convexHull(p_left, p_right, x, y, flag, dist, segm, N-2);
	pStop = seconds() - pStart;
	printf("\n\tSTEP 9: %f sec \n\n", pStop);
	...
	//printConvexHullDim();
	printConvexHull();
	printf("\n\tNo pre-processing Quickhull GPU elapsed time %f sec \n\n", stopGPU);
